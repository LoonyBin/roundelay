...
| `ext_class_not_active` | 422 | `POST …/ops` extension classes | `index`, `op_class` | yes | Log |
| `ext_class_not_bound` | 422 | `POST …/ops` ext_binding | `index`, `op_class` | yes | Log |
...
| `keywrap_requires_owner` | 403 | `PUT …/keywraps` | `revoked` | yes | Keys |
...
| `malformed_prune_payload` | 422 | `POST …/ops` prune | `index` | yes | Log |
...
| `prune_target_attestation_mismatch` | 422 | `POST …/ops` prune | `index`, `seq` | yes | Log |
...
| `unknown_member` | 404 | `POST /v1/members/{m}/challenge` | — | yes | Identity |
...
**[S]** `cert_root_pk_mismatch` is raised on a genesis whose `root_pk` does not derive
the Workspace, and on a handover whose `from_root_pk` is not the Workspace's current
Root. Both mean **the Root this certificate names is not the one in force**, and both
are repaired the same way: re-read the log and rebuild the document against the Root
it actually reports.
...
> same thing; only the route it arrived by differs, and that is already in the
> response.
